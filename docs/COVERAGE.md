...
| `--with-analysis` | false | Enable build processing service integration (screenshots + metadata ZIP). Enabled by default in generated workflows. |
...
## Build Processing Service Integration

When `--with-analysis` is enabled, the deployer runs additional steps after coverage analysis:

1. **Playwright execution**: Launches a headless browser to capture screenshots of each Storybook story
2. **Screenshot capture**: Saves PNG screenshots for every story in the build
3. **Metadata ZIP generation**: Creates a ZIP containing screenshots and story metadata (`metadata.json`)
4. **Upload**: The metadata ZIP is uploaded alongside the Storybook deployment for LLM analysis by the build processing service

This is powered by `@scrymore/scry-sbcov` v0.3.0+.

### Generated workflows

The `--with-analysis` flag is **enabled by default** in workflows generated by `scry-deployer init`. To disable it:

- Set the GitHub Actions variable `STORYBOOK_DEPLOYER_WITH_ANALYSIS` to `false`, or
- Remove the `--with-analysis` line from your workflow file
