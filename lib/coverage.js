...
const { getGitContext } = require('./git-context.js');
const { addFingerprintsToReport } = require('./fingerprint.js');
const { computeTransitions } = require('./transitions.js');
 ...
 * @property {string} [previousReportPath] Optional path to a previous coverage report for transitions
 */
...
async function runCoverageAnalysis(options) {
  const { storybookDir, baseBranch = 'main', failOnThreshold = false, execute = false, outputPath: providedOutputPath, keepReport = false, previousReportPath } = options || {};
  ...
  const resolvedPreviousReportPath = previousReportPath
    ? (path.isAbsolute(previousReportPath) ? previousReportPath : path.resolve(process.cwd(), previousReportPath))
    : null;
  ...
  try {
    ...
    // Enhance report with git context for GitHub issue creation
    const reportWithGitContext = enhanceReportWithGitContext(report);

    // Add stable story fingerprints for cross-build correlation
    const enhancedReport = addFingerprintsToReport(reportWithGitContext);

    const previousReport = resolvedPreviousReportPath ? loadCoverageReport(resolvedPreviousReportPath) : null;
    const transitions = previousReport ? computeTransitions(previousReport, enhancedReport) : null;

    const finalReport = transitions
      ? { ...enhancedReport, transitions }
      : enhancedReport;
...
/**
 * Enhance a coverage report with git context information.
 *
 * This adds commitSha, branch, buildUrl, buildId, prNumber, and commitUrl
 * to the report for GitHub issue creation and cross-build correlation.
 *
 * @param {any} report - The coverage report to enhance
 * @returns {any} - The enhanced report with gitContext field
 */
function enhanceReportWithGitContext(report) {
  ...
  // Only add gitContext if we have at least some data
  const hasData = Object.values(gitContext).some((v) => v !== null);
  if (!hasData) {
    return report;
  }
