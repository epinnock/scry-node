...
async function uploadFile(presignedData, filePath) {
  ...
  try {
    // If fields are present, use presigned POST (S3 style), otherwise use PUT
    if (presignedData.fields && Object.keys(presignedData.fields).length > 0) {
      ...
      await axios.post(presignedData.url, form, {
        headers: form.getHeaders(),
        maxContentLength: Infinity,
        maxBodyLength: Infinity,
      });
    } else {
      // Fallback to PUT for simple presigned URLs
      await axios.put(presignedData.url, fileBuffer, {
        headers: {
