 ...
 * @param {string} payload.branch The Git branch.
 * @param {string} payload.commitSha The Git commit SHA.
 * @returns {Promise<string>} A promise that resolves to the presigned upload URL.
 */
async function requestPresignedUrl(apiClient, { branch, commitSha }) {
  // This is a mock check to allow testing of a 500 server error.
  if (branch === 'fail-me-500') {
  ...
  if (branch === 'fail-upload') {
    return 'https://s3.amazonaws.com/fake-bucket/fail-upload';
  }

  try {
    const response = await apiClient.post('/deploy/presigned-url', {
        branch,
        commitSha,
    });
